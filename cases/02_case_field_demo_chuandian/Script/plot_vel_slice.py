...
Y = Y_all[1:-1]
Z = Z_all[1:-1]
vp_data = np.loadtxt(vp_file).reshape(nz, ny, nx)[1:-1,1:-1,1:-1]
vs_data = np.loadtxt(vs_file).reshape(nz, ny, nx)[1:-1,1:-1,1:-1]
vpvs_data = np.loadtxt(vpvs_file).reshape(nz, ny, nx)[1:-1,1:-1,1:-1]
...
def plot_slice(data3d, Z, X, Y, slice_dep, X_uni, Y_uni, cmap, vlabel, out_prefix, interp_method='cubic',
               event_x=None, event_y=None, event_dep=None, event_dep_range_km=1.0):
...
if __name__ == "__main__":
    for interp_method in ['cubic']:  # 只使用cubic插值
        plot_slice(vp_data, Z, X, Y, slice_dep, X_uni, Y_uni, cmap_vp, 'Vp (km/s)', f'{outdir}Vp_slice', interp_method=interp_method,
                   event_x=event_lon, event_y=event_lat, event_dep=event_dep, event_dep_range_km=event_dep_range_km)
        plot_slice(vpvs_data, Z, X, Y, slice_dep, X_uni, Y_uni, cmap_vpvs, 'Vp/Vs', f'{outdir}VpVs_slice', interp_method=interp_method,
                   event_x=event_lon, event_y=event_lat, event_dep=event_dep, event_dep_range_km=event_dep_range_km)
        plot_slice(vs_data, Z, X, Y, slice_dep, X_uni, Y_uni, cmap_vpvs, 'Vs (km/s)', f'{outdir}Vs_slice', interp_method=interp_method,
                   event_x=event_lon, event_y=event_lat, event_dep=event_dep, event_dep_range_km=event_dep_range_km)
