c--- in this version, output abs.dat, dt.ct, and dt.cc respectively.
	...
	subroutine syn_time_new_SP(log, ndt, nsrc, nsta, 
...
c	Local variables:
	...
	real            qual1, qual2
        ...
        write(log,'("~ getting synthetic data files for S-P data...")')
...
c        c=0.03 ! Control the random noise level for S wave
	...
	call freeunit(log1)
	open(log1, file='dt_sp.syn', status='unknown')
	call freeunit(log2)
	open(log2, file='absolute_sp.syn', status='unknown')
	call freeunit(log3)
	open(log3, file='dtcc_sp.syn', status='unknown')
...
c       Mulitple sources
	...
	do i=1,ndt
	   if (dt_idx(i).eq.3) then ! catalog data
c       P phase
