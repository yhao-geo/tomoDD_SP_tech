...
c--- get rms:
      ...
      rms_ccold= rms_cc
      rms_ctold= rms_ct
!      write(*,*) 'rms_ct0old,rms_ctold: ',rms_ct0old,rms_ctold
     
      j= 0
      sw_cc= 0
      sw_ct= 0
      do i= 1,ndt
!         write(*,*) 'i,idx(i),w(i),d(i): ',i,idx(i),w(i),d(i)
         if(idx(i).le.2) then
             sw_cc= sw_cc + w(i)
             j= j+1
         else
      ...
      enddo
c      f_cc= j/sw_cc  ! factor to scale weights for rms value
      if(j.gt.0) then
...
!             write(*,*)'i,d(i),rms_ct0: ',i,d(i),rms_ct0
...
!      write(*,*) 'rms_ct0,av_ct0,rms_ct,av_ct: ',rms_ct0,av_ct0,rms_ct,av_ct
c      av_cc0= av_cc0/j
      av_ct0= av_ct0/(ndt-j)
c      rms_cc0= sqrt( (rms_cc0 - av_cc0**2/j) / (j-1) )
      rms_ct0= sqrt( (rms_ct0 - av_ct0**2/(ndt-j)) / (ndt-j-1) )
c      av_cc= av_cc/j
      av_ct= av_ct/(ndt-j)
c      rms_cc= sqrt( (rms_cc - av_cc**2/j) / (j-1) )
