...
c-- Loop to read each parameter lines, skipping comments
 210  read (fu_inp,'(a)',end=220) line
      ...
      if (l.eq.8) read (line,'(a)',err=999) fn_initial
      ...
      if (l.eq.14) read (line,'(a)',err=999) fn_abs
      if (l.eq.15) read (line,*,err=999) idata, iphase,maxdist
      if (l.eq.16) read (line,*,err=999) minobs_cc,minobs_ct, air_dep
      if (l.eq.17) then 
...
c--Read iteration instructions
      if (l.ge.21 .and. l.le.20+niter) then
         i=l-20
...
c--Read specific clusters/events to relocate
      if (l.eq.21+niter) read (line,*,err=999) iclust
      if (l.ge.22+niter) then
