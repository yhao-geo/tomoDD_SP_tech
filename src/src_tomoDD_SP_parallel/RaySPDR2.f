...
c     Firstly determine number of layer
      ...
      do while(lo.LT.0.0e10)
         lo=lo+360.00
      enddo
      if (abs(lo).gt.180) lo = lo-lo/abs(lo)*360
...
c      write(*,*) 'x,y,z:',x,y,z
      ...
      if (ip.lt.1 .or. jp.lt.1 .or. kp.lt.1) then
         ! this ray point is out of the boundary.
         ! To continue running the main program, we manually set the velocity of this ray point.
         ! And rayError=1 will make the main program to cancle this wrong travel time.   --Hao Guo.
         rayError1 = 1
         v = 1
...
c
      ...
      jp1=jp+1
      kp1=kp+1
!     write(16,100)x,xl,ip,y,yl,jp,z,zl,kp
      if(ip.eq.0 .or. jp.eq.0 .or. kp.eq.0) then
         write(*,*)'Warning: ip,jp, or kp =0'
         write(*,*)x,xl,int((x+xl)/bld),ip
         write(*,*)y,yl,int((y+yl)/bld),jp
         write(*,*)z,zl,int((z+zl)/bld),kp
      endif
...
c      kp=izloc(kp)
      ...
      if (ip.lt.1 .or. jp.lt.1 .or. kp.lt.1) then
        ...
        write(*,*) 'intmap',rayError1
         ...
         jp=iyloc(jp)
         !if(kp.le.0) then
         !  write(*,*)'Warning: z position off map'
         !  write(*,*)'intmap: ',x,y,z,kp,xl,yl,zl,bld
         !  write(*,*)'Setting kp = 0'
         !  kp=0
         !endif
         kp=izloc(kp)
...
!ip,jp,kp=0,in() is smaller than 0     ----wangfan 4.8
            if(ip .lt. 1 .or. jp .lt. 1 .or. kp .lt. 1) then
              in(1)=1
              in(2)=1
              in(3)=1
...
! ---wangfan
                       if(in(ijk).lt.0) write(*,*) ijk,in(1),in(2),in(3),in(4),in(5),in(6),in(7),in(8),'ip:',ip,'jp',jp,'kp',kp
                       if(in(ijk).lt.0) write(*,*) 'Station:',aar0,bbr0,'Event:',aas0,bbs0,'point:',tlat,tlon
                       if(in(ijk).lt.0) write(*,*) 'dxs:',dxs,'dys:',dys,'dzs:',dzs
                       if(in(ijk).lt.0) write(*,*) 'nseg:',nseg,'fnsegi:',fnsegi,'i',i,'rayError:',rayError1
                       if(in(ijk).lt.0) write(*,*) rp(1,i),rp(2,i),rp(3,i),rp(1,i+1),rp(2,i+1),rp(3,i+1)
