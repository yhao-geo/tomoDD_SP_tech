...
c--- define coordinate center and the size of the box (xd,yd)
     ...
     &       fn_loc,fn_reloc,fn_initial,fn_res,fn_stares,fn_srcpar,
...
c           call dtres_fdd(log,ndt,nsrc,nsta,
c     &          dt_dt,dt_idx,
c     &          dt_ista,dt_ic1,dt_ic2,
c     &          src_t,tmp_ttp,tmp_tts,
c     &          dt_cal,dt_res, eve_sta)


c---  output initial residuals: mdat.res   111111111111111111111111111111111111111111111111111111111111111111111
c      if(trimlen(fn_initial).gt.1) then
c         call freeunit(iunit)
c         open(iunit,file=fn_initial,status='unknown')
c         write(iunit,'("STA",11x,"DT",8x,
c     &        "C1",8x,"C2",4x,"IDX",5x,"QUAL",4x,"RES [ms]",3x,"WT",9x,
c     &        "OFFS")')
c         write(iunit,'(a7,1x,f12.7,1x,i9,1x,i9,1x,i1,1x,
c     &        f9.4,1x,f12.6,1x,f11.6,1x,f8.1)')
c     &        (dt_sta(j),dt_dt(j),dt_c1(j),dt_c2(j),dt_idx(j),dt_qual(j),
c     &        dt_res(j)*1000,dt_wt(j),dt_offs(j),j=1,ndt)
c         close(iunit)
c      endif
c      stop
